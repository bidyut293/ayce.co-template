...
(function () {
  $(".animsition").on("animsition.inEnd", function () {
    AOS.init({
      duration: 1000,
      // duration: 200, // Set the default duration globally
      // offset: 50, // Start animation when 50px inside viewport
      // easing: "linear", // Use a faster easing
...
(function () {
  ...
  async function includeHTML() {
    ...
    const cache = {}; // Cache to avoid redundant fetches
    const activeIncludes = new Set(); // Track active files to prevent recursion

    for (const el of elements) {
      const file = el.getAttribute("data-include");
      if (!file) continue; // Skip elements without `data-include`

      try {
        // Detect recursive inclusion using the `activeIncludes` set
        if (activeIncludes.has(file)) {
          throw new Error(`Recursive inclusion detected for: ${file}`);
        }

        // Mark the file as being processed
        activeIncludes.add(file);

        // Fetch file content (cache first)
        const content = cache[file] || (cache[file] = await fetchHTML(file));
        el.innerHTML = content; // Inject the fetched content

        // Look for nested includes in the newly added content
        const nestedElements = el.querySelectorAll("[data-include]");
        if (nestedElements.length > 0) {
          await includeHTML(); // Recursively process nested includes
        }
      } catch (error) {
        console.error(error);
        el.innerHTML = `<div style="color: red;">Error loading ${file}: ${error.message}</div>`;
      } finally {
        // Remove the file from the active includes set
        activeIncludes.delete(file);
      }
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    let feedbackContainer = form.querySelector(".feedback-container");
    ...
    try {
      ...
      // Custom success message based on the form action
      let successMessage = "We have received your request."; // Default message
  ...
  // Attach event listeners to all forms dynamically
  ...
  function equalizeHeights() {
    let maxHeight = 0;
    let cards = document.querySelectorAll(".slide_card");

    cards.forEach((card) => {
      card.style.height = "auto"; // Reset height to recalculate
